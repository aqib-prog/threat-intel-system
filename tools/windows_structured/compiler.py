#!/usr/bin/env python3
"""Card 5 Part 1 step 7: compile Windows field-aware Sigma conditions."""
...
DEFAULT_SPECS = HERE / "windows_structured_rule_specs.py"

for path in (str(BACKEND), str(SIGMA_TOOLS)):
    if path not in sys.path:
        sys.path.insert(0, path)

from log_analysis.parser import WINDOWS_CANONICAL_ALIASES  # noqa: E402
...
def compile_windows_structured(
    ...
    collection = SigmaCollection.load_ruleset(
        [sigma_root / "rules/windows"], collect_errors=True
    )
    ...
    inventory = {
        "approved_windows_sigma_candidates": len(approved),
        "structured_windows_candidates": len(specs),
        "raw_fallback_only_candidates": len(review),
        "canonical_windows_field_count": len(WINDOWS_CANONICAL_ALIASES),
...
def render_specs(
    ...
    return (
        '"""Generated Card 5 step-7 Windows structured rule specs.\n\n'
...
def main() -> int:
    ...
    report = {
        "checkpoint": "Card 5 Part 1 roadmap step 7 Windows pilot only",
        "sigma_commit": step2_report["sigma_commit"],
        "hybrid_policy": (
            "A structured condition is authoritative for a complete Windows JSON "
            "record, or for a partial record once one full positive field branch is "
            "available; otherwise that rule/event falls back to its raw regex."
        ),
        "inventory": inventory,
