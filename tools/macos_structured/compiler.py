#!/usr/bin/env python3
"""Card 5 Part 1 step 8: compile macOS field-aware Sigma conditions."""
...
DEFAULT_SPECS = HERE / "macos_structured_rule_specs.py"

for path in (str(BACKEND), str(SIGMA_TOOLS)):
    if path not in sys.path:
        sys.path.insert(0, path)

from log_analysis.parser import MACOS_CANONICAL_ALIASES  # noqa: E402
...
def compile_macos_structured(
    ...
    approved = [
        item for item in step2_report["mapping_candidates"] if item["platform"] == "macos"
    ]
    approved_by_path = {item["source_path"]: item for item in approved}
    collection = SigmaCollection.load_ruleset(
        [sigma_root / "rules/macos"], collect_errors=True
    )
    ...
    inventory = {
        "approved_macos_sigma_candidates": len(approved),
        "structured_macos_candidates": len(specs),
        "raw_fallback_only_candidates": len(review),
        "canonical_macos_field_count": len(MACOS_CANONICAL_ALIASES),
...
def render_specs(specs: dict[tuple[str, str], dict[str, Any]], sigma_commit: str) -> str:
    return (
...
def main() -> int:
    ...
    step2 = json.loads(args.step2_report.read_text(encoding="utf-8"))
    specs, review, inventory = compile_macos_structured(args.sigma_root, step2)
    report = {
        "checkpoint": "Card 5 Part 1 roadmap step 8 macOS pilot only",
        "sigma_commit": step2["sigma_commit"],
        "hybrid_policy": (
            "A structured condition is authoritative for a complete macOS ECS JSON "
            "record; malformed/non-JSON records retain per-rule raw fallback."
        ),
        "inventory": inventory,
    ...
    args.specs.write_text(render_specs(specs, step2["sigma_commit"]), encoding="utf-8")
