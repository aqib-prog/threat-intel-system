#!/usr/bin/env python3
"""Build the reviewed Windows Sigma runtime bundle from committed artifacts."""
...
DEFAULT_STRUCTURED_SPECS = (
    REPO / "tools/windows_structured/windows_structured_rule_specs.py"
)
DEFAULT_OUTPUT = (
    REPO / "backend/log_analysis/generated/windows_sigma_rules.json.gz"
)
...
def build_payload(
    ...
    for item in sigma_specs.RULE_SPECS_BY_PLATFORM["windows"]:
...
def main() -> int:
    ...
    sigma = load_module(args.sigma_specs, "runtime_windows_sigma_specs")
    structured = load_module(
        args.structured_specs, "runtime_windows_structured_specs"
    )
