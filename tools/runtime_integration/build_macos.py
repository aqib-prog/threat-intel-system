#!/usr/bin/env python3
"""Build the reviewed macOS Sigma runtime bundle from committed artifacts."""
...
DEFAULT_STRUCTURED_SPECS = REPO / "tools/macos_structured/macos_structured_rule_specs.py"
DEFAULT_OUTPUT = REPO / "backend/log_analysis/generated/macos_sigma_rules.json.gz"
...
def build_payload(
    ...
    for item in sigma_specs.RULE_SPECS_BY_PLATFORM["macos"]:
...
def main() -> int:
    ...
    sigma = load_module(args.sigma_specs, "runtime_macos_sigma_specs")
    structured = load_module(args.structured_specs, "runtime_macos_structured_specs")
